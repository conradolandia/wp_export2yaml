...
def main():
    parser = argparse.ArgumentParser(
        description="Convert WordPress WXR export to YAML format"
    )
    parser.add_argument("xml_file", help="Path to WordPress XML export file (.wxr)")
    parser.add_argument(
        "yaml_file", help="Path where the YAML output file will be saved"
    )
    parser.add_argument(
        "--post-types",
        nargs="+",
        help="List of post types to include (space-separated)",
    )
    parser.add_argument(
        "--exclude-custom-fields",
        nargs="+",
        help="List of custom fields to exclude (space-separated)",
    )
    parser.add_argument(
        "--convert-to-markdown",
        action="store_true",
